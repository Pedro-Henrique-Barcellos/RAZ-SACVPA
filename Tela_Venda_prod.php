...
<body class="fundo">
    <div class="letras">
        <form action='CRUD/cad_venda_prod.php' method='POST'>
            <h3>Vender <?php echo $nome?></h3>
            
            <input type='hidden' name='produto_venda' value="<?php echo $id_prod ?>">
            <input type="hidden" name="preco_total_vendas" value ="<?php echo $preco?>">

            <label for='quant_venda'>Quantidade:</label>
            <input type='number' placeholder="maximo: <?php echo $max?>" name='quant_venda' required id='quantity' min=1 max="<?php echo $max?>" oninput="updatePriceLabel(this.value)">
            
    
            <br>
            <label id="priceLabel">Total: $0.00</label>
            <br>           


            <label for='cliente_venda'>Cliente:</label>
            <select name="cliente_venda" id="" required style="color:white; width:300px;" >
            ...
            <a href="table.php"><input style=width:100px class = "custom-btn btn-2" value=VOLTAR></input></a><span style="color:rgba(0,0,0,0)">--------</span><a  href=""><input type='submit' style=width:100px class="custom-btn btn-2" value='VENDER'></a>
        </form>
    </div>
    
    <script>
    function updatePriceLabel(quantity) {
        const priceLabel = document.getElementById("priceLabel");
        const price = quantity * <?php echo $preco ?>; // Example calculation based on quantity
        
        priceLabel.textContent = `Total: $${price.toFixed(2)}`;
    }
    </script>
</body>
</html>
